...
#### Session 6.3 — Scoped zones compiler feature (CLOSED 2026-05-07)

`@profile("name") { ... }` shipped — the spec wanted
`@profile_zone(...)` but `_zone` was redundant (the annotation
*is* the zone), so the surface dropped a syllable. Three
moving pieces:

- **Parser** (`src/bpp_parser.bsm`): `parse_statement`
  recognises the annotation and lowers it at parse time to a
  synthesized `T_BLOCK` carrying `T_CALL _prof_zone_enter`,
  the user's body, and `T_CALL _prof_zone_exit`. Reuses the
  existing T_LIT + T_CALL builders so codegen treats the
  prologue/epilogue identically to hand-written calls.
  Surfaced one generic compiler gap: `pre_reg_vars` did not
  recurse into `T_BLOCK`, so any `auto` declarations inside a
  zone went unregistered and the linker errored with
  `internal error: global '<name>' not found in data section`.
  Fixed in both backends (commit `7871ba3`).
- **Runtime** (`stb/stbprofile.bsm`): flat 16-slot zone
  table (name_ptr / total_us / count) + 8-deep open-zone
  stack, both zero-init in `init_profile`. Public surface:
  `_prof_zone_enter / _prof_zone_exit` (called from the
  synthesised T_CALLs), `profile_zones_reset`,
  `profile_zones_hud_draw`. The HUD panel gates on
  `_profile_hud_active` so it surfaces / dismisses with the
  rest of the profile HUD via the same key press.
- **Smoke** (`examples/profile_zones_smoke.bpp`) — three
  zones of measurable busy work (heavy / medium / light)
  produce a sorted-by-total-µs table within ~1 second.

fps_3d_gpu adopted the syntax in the same commit: render phase
wraps `ray_cast / hud_overlay / crt_effect` in @profile blocks,
zones panel pinned to the upper-right corner. The original gate
("profile zone 'render_pass' appears in top-N") shifted to
per-pass instead — same intent, finer grain.

Caveats baked into the v1:
- Early `return` inside a zone leaves it open. Future cleanup
  pass will tighten via scoped epilogues.
- Nested zones run on the runtime stack; aggregation is flat
  (parent total includes children).
- Panic / abort similarly leaks. Out of scope for v1.

Tonify Rule 25 added — `@profile` annotation usage + the v1
caveats — see `docs/tonify_checklist.md`.
...
profile breakdown via scoped zones — **MET** (Session 6.3
shipped 2026-05-07; ray_cast / hud_overlay / crt_effect
zones land in `fps_render_phase`).
