#!/usr/bin/env python3
"""Build and test the B++ bootstrap parser."""
...
SOURCE = os.path.join(SCRIPT_DIR, "parser.bpp")
SHIM_C = os.path.join(SCRIPT_DIR, "parser_shim.c")
OUTPUT = os.path.join(SCRIPT_DIR, "bpp_parser")
...
def translate_node(node):
    ...
    # Local variable declaration inside control-flow blocks.
    if t == "decl":
...
def build():
    """Compile parser.bpp to a native executable."""
    print(f"{CYAN}🔧 B++ Bootstrap Parser Builder{RESET}")
    ...
    # Suppress warnings: -w for general, -Wno-int-conversion for string
    # literals passed to long long parameters (safe on 64-bit platforms).
    ...
    print(f"{GREEN}Parser built: {OUTPUT}{RESET}")
    return True


def test(input_file=None):
    """Run lexer + parser pipeline on a test input and display the IR."""
    ...
    if input_file:
        print(f"\n{CYAN}Parsing: {input_file}{RESET}\n")
        with open(input_file) as f:
            lex = subprocess.run(
                [LEXER], stdin=f, capture_output=True, text=True)
    else:
        test_code = (
            'auto x;\n'
            'main() {\n'
        ...
        print(f"\n{CYAN}Parsing inline test:{RESET}\n{test_code}")
        lex = subprocess.run(
            [LEXER], input=test_code, capture_output=True, text=True)

    if lex.returncode != 0:
        print(f"{RED}Lexer error:\n{lex.stderr}{RESET}")
        return

    result = subprocess.run(
        [OUTPUT], input=lex.stdout, capture_output=True, text=True)
    ...
    try:
        ...
        if not result.stdout.strip():
            print(f"{RED}Parser produced no output.{RESET}")
