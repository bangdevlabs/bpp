...
- **WC2 worker AI solved by porting Stratagus, not guessing.** The rts2 mod's peasants jammed at every resource; reading the actual Stratagus source (`action_resource.cpp` + the A* `CostMoveToCallBack`) gave the real model in one pass. `stbpath` gained a generic **dynamic-obstacle hook** (route around parked units, cross moving ones; suite 183/0/12), plus per-tree / perimeter spread and `MoveToResource` re-path on every leg — the economy now flows. Same session: native 640×480 field of view (the Blizzard-remaster technique) and the WC2 portrait-icon strip.
...
- **SIMD — explicit + autovec** — 11 float `vec_*` builtins (4×f32) plus 4 byte `vec_*` builtins (16×u8: `vec_load16b` / `vec_splat_byte` / `vec_cmeq_byte` / `vec_movemask`, the group-scan kit behind `buf_find_byte`) lower to NEON (ARM64), SSE2 (x86_64), or a portable form under `--c`; the autovec pattern matcher recognises `arr[i] = arr[i] OP literal` shapes and emits a 4-wide SIMD loop + scalar tail automatically. `auto v: double` allocates from the SIMD register pool
...
| **May 29** | **The second dog-food lap: `: float` infected mixed `auto`, the disassembler caught it while producing FPS.

 **Jun 01-03** | **Funnel conversor used in 3 different games, converting differente kinds of assets into Bang9 line of production. Funnel_conversor.md created for reference on how to use. 

| **Jun 04-06** | **rts2 (WC2 mod): native FOV, portraits, and the gather-contention saga solved by porting Stratagus. Worker repair (player + AI), and the wc1→wc2 rename. Refactoring stbpath & RTS2 Movement Architecture (WC2 Mod) - Additive Pathfinding, Regression Testing, and Thrashing Reduction.

| **Jun 10** | **A learner's book — and two codegen bugs it flushed out.** Started *The B++ Programming Language*, transposing Kernighan & Ritchie chapter by chapter into B++ (Chapter 1: 13 runnable examples + 7 self-checking exercises + a gate that compiles *and runs* every listing). Writing the smallest honest examples surfaced two latent codegen bugs before a single exercise was assigned: the inliner corrupted a caller when a function wrote its parameter (it substituted a written parameter with the caller's variable — fixed by forcing the per-callsite-local BIND path), and `put` mis-printed inside a `switch` arm (the type pass walked `T_IF`/`T_WHILE`/`T_BLOCK` but not `T_SWITCH`). Both fixes byte-stable, suite 183/0/12. A textbook, it turns out, is a conformance suite. |

| **Jun 11** | **The book drove a language feature: variadic `put`.** Chapter 1's `putstr("count: "); putnum(n); putchar('\n')` boilerplate was the tell — so `put` (and `put_err`) now take any number of arguments, each smart-dispatched by type: `put("count: ", n, "\n")`. Parser-only (a `put(...)` statement with ≥2 args expands into one single-arg `put` per argument), so codegen / validator / backends are untouched; the per-arg ambiguity warning is suppressed inside the list (a value there is clearly meant to print). No `println`, no format string — the natural generalisation of a name the reader already knows (new Tonify Rule 45). Byte-stable, native 185/0/12, C-emit 150/0/47. |
| **Jun 13-14** | **The audio codegen arc — register allocation + the arity-8 register-window return.** Compute-in-place + float promotion + `fmadd` take the canonical biquad IIR to **1.48x — now beating gcc -O0** (0.769s vs 0.927s), and multi-value return ships the arity-8 *register-window transformer* ABI (`l, r = filter(l, r)` keeps the stereo pair register-resident across the chain). Cross-platform verified in Docker (x86_64 self-host byte-stable + a clean E266 guard). The discipline was the story: three "obvious" wins — including the integer-loop lever — were each *measured* to ~0% and skipped. Self-hosts in 0.25s; native 188/0/12. Full narrative in [the journal](docs/journal.md). |
| **Jun 16** | **Byte-wide SIMD across all three backends — and a latent Mach-O bug it flushed out.** New `vec_load16b` / `vec_splat_byte` / `vec_cmeq_byte` / `vec_movemask` builtins (constant-free SRA-tree movemask on a64, `PMOVMSKB` on x64, a portable scalar struct under `--c`) power a `buf_find_byte` prelude primitive — a SIMD memchr **11x (a64) / 9.6x (x64)** over the byte loop. x64 also reached full **multi-value-return parity** (a custom 8-wide register window — `rax/rdx/rcx/rsi/rdi/r8-r10` + `xmm0-7` — not SysV's 2-register limit). Growing the compiler past 1 MB exposed a years-latent Mach-O bug: the chained-fixups `datasize` precompute hardcoded its one-page value, so any binary spanning >1 `__DATA` page was malformed (`dyld: string overflow`) — **debugged the compiler with itself** (`bug --break a64_macho.bsm`, declared 432 vs actual 444) and fixed foundation-first. Plus SwissTables *measured* against linear and **shelved** (1.5–2.1x slower at our sparse load — the industry runs dense, we run sparse on purpose), and `fmadd`/F.2.c documented as an architectural x64 opt-out (SysV has zero callee-saved xmm). Native 192/0/12, C-emit 155/0/49. Full narrative in [the journal](docs/journal.md). |
| **Jun 16** | **Integer compute-in-place — closing the AAA inner-loop gap.** A committed codegen-quality gate (`bench_codegen.bpp`) measured a throughput kernel **3.71x behind gcc -O2** and a controlled experiment decomposed it: accumulator shuffle ~58%, constants ~29%, params ~13% — so the real lever is the `x0` funnel, not hoisting. Three stages, each byte-stable: **(1)** `cg_emit_int_into` emits `+ - * & | ^` integer trees (over local / constant / `arr[i]` leaves) destination-driven instead of routing every value through `x0`, the integer analog of the float F.2.c; **(2a)** loop-weighted **constant promotion** lifts hot loop-invariant constants into spare callee-saved registers, materialised once in the prologue; **(2b)** loop-weighting the B3 ref-count so a loop-invariant param (an array base, the loop bound) used once per iteration promotes instead of reloading from the frame. Cumulative: the throughput kernel **22.9 → 14.6ms (3.71x → 2.35x)**, lcg 24.3 → 20.8ms (1.35x → 1.14x); the residual was first blamed on gcc's auto-vectorisation — corrected Jun 17 (it was scalar addressing). Compile-time 0.25s unchanged; native 192/0/12, C-emit 155/0/49; x64 self-host stable. |
| **Jun 17** | **Instruction selection — and the disassembly that overturned the premise.** Disassembling gcc -O2 *first* showed its xform loop is six scalar instructions (`ldr [x0],#8; madd; asr #17; …`) — **no SIMD, no unrolling** — so the gap was never the register allocator (the 6-register file was enough) and never auto-vectorisation. It was three missing instruction-selection features and one dead-code bug: **immediate shift** (`asr #17` not `mov #17; asrv`); a **CIP memory-load leaf fix** (it checked `n.b != 8`, but subscripts carry the field-hint *type* 0 — so `a[i]` trees had silently fallen to the accumulator path since Stage 1, funnel and all); **strength reduction** (`i*8` → `lsl #3`); and **integer `madd`/`msub` fusion** (the integer twin of fmadd). The serial **lcg reached gcc -O2 parity** and the throughput xform fell **2.35x → ~1.5x** (22 → 9 loop instructions). The remaining gap is purely addressing — gcc walks pointers where we recompute the index. Four byte-stable commits; native 192/0/12, C-emit 155/0/49; x64 self-host stable. Full narrative in [the journal](docs/journal.md). |
| **Jun 17** | **Register allocation, finished — and the pointer walk that closed the addressing gap.** Three more commits took "professionalise the register allocator" to its conclusion. **x64 constant promotion** reached a64 parity (and unblocked a real convention bug — promoted regs are physical, not return-bank slots — via a new `emit_mov_phys`). **Wider register budget:** a leaf function (no calls) now also promotes locals into caller-saved registers, lifting the budget 6 → 10 — a register-heavy leaf kernel hit **gcc -O2 parity** (byte-equal output). **Induction-variable pointer walk:** a counted `for (i…) base[i]` now walks the base's register as a pointer instead of recomputing `base+i*8`, collapsing the xform loop body to **gcc's exact four instructions** (`ldr; madd; asr; str`), gated airtight by a whole-function safety scan. Cumulative this session: the throughput xform fell **2.35× → ~1.3× gcc -O2**, with the serial lcg and the leaf kernel both at parity. The lesson: the AAA kernel was never register-*pressure* bound (6 registers fit) — b++ already had an allocator; the work was widening and extending it. Native 192/0/12, C-emit 155/0/49; x64 self-host stable; 0 bootstrap warnings. |
| **Jun 17** | **Loop control — and a benchmark book.** Two general peepholes closed the last of the xform gap: a **self-update with an immediate** (`i++` drops from three instructions to `add x19,x19,#1`) and **compare-and-branch fusion** (a loop/if condition branches straight on the flags — `cmp; b.ge` — instead of materialising a boolean with `cmp; cset; cbz`, using the inverse condition code `cc^1`). Both land on every loop and every `if` in the codebase, on both backends. The throughput **xform fell 1.44× → ~1.1× gcc -O2** — within noise of the oracle, loop body *and* control now matching its structure. Closed with `docs/manual/benchmarks.md`, the consolidated catalog + historical record for every perf benchmark (compile time, codegen, autovec/compose, ECS, tablah). Native 192/0/12, C-emit 155/0/49; x64 self-host stable; 0 bootstrap warnings. |
| **Jun 17** | **The sweet spot — integer codegen at `gcc -O2` parity.** Two final register levers, both extensions of the existing compute-in-place: **assign into the destination register** (`s = s*A + C` emits one `madd s, s, A, C`, no copy-back) and **compare promoted registers directly** (`while (i < n)` → `cmp x19,x21; b.ge`, no scratch copies). The lcg loop is now five instructions, the same shape gcc emits: **lcg 1.02× (parity), xform 1.10×**. With its own from-scratch backend, the integer codegen reached the oracle in a span of days — from 3.71× a week ago. Then the float-serial kernel followed: a **float assign-into-destination** (the float twin of the same lever) killed the `fmov`-through-d0 funnel in the biquad state updates and took it **1.49× → 1.02× — parity, Phase 1 alone**; the FP scheduler (Phase 2) was *deferred*, not built, because the measured gap was gone. **All three bench kernels — integer serial, integer throughput, float serial — now match `gcc -O2`.** Codegen is no longer the bottleneck; a market self-assessment vs C/C++, Jai, Odin, Zig is in `docs/manual/positioning.md`. Native 192/0/12, C-emit 155/0/49; x64 self-host stable. |
