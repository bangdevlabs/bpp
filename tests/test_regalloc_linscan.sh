...
echo "$DUMP" | grep -A4 "ASN __rg_asn_straightline" | grep -q "var 0: slot 0" || fail "straightline var 0 (a): expected slot 0"
echo "$DUMP" | grep -A4 "ASN __rg_asn_straightline" | grep -q "var 3: slot 0" || fail "straightline var 3 (y): expected slot 0 (a's slot, freed when a expired)"
...
LOOP_BLOCK=$(echo "$DUMP" | grep -A6 "ASN __rg_asn_loop")
...
NONOVERLAP_BLOCK=$(echo "$DUMP" | grep -A5 "ASN __rg_asn_nonoverlap")
