...
| E265 | struct exceeds the per-struct field limit | bpp_parser.bsm | add_struct_field | ✅ | A `struct` declares more than `_SD_MAX_FIELDS` (256) fields. The per-struct field tables (`sd_fields` / `sd_fhints` / `sd_byte_offs` / `sd_bit_offs`) are fixed-capacity `buf_word(256)` allocations; `add_struct_field` now bounds-checks `fc` against the limit and emits this error instead of writing past the buffer. Caught while bringing up the size-class free-list allocator (2026-05-27): `ChipPrimitives` (136 fields) was already within the old `buf_word(64)` overflow by 72 words, but the mmap-per-alloc page slack absorbed the stray writes; once the free-list packed allocations tightly the overflow corrupted the neighbouring allocation. Fix raised the capacity to 256 and added the gate so any future over-large struct is a clean compile error, not silent corruption. Help message prints the struct name and the limit. |
