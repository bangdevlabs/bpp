...
| E266 | multi-value return/assignment needs register-bank support | bpp_codegen.bsm | 4956 + 5043 | ❌ | `return a, b;` or `a, b = f()` compiled for a backend whose `cap_multi_ret` primitive answers 0. Both shipped chips (a64, x64) implement banking, so today this fires only for a future backend bring-up that has not filled the capability yet. |
| E267 | float literal in a constant expression the integer evaluator cannot fold | bpp_parser.bsm | const_eval_node T_LIT | ❌ | Know-or-refuse doctrine (2026-07-03): `const X = 2.5 * 2.0;` (float ARITHMETIC in a const RHS, float enum member, etc.). The single-literal shapes — positive and negated — take the dedicated float fast paths and work; every other decimal shape used to be "evaluated" by re-parsing the literal's bytes with '.' as digit -2, producing silent garbage (`-0.25` became 175). The evaluator now refuses instead. Help line points at the two workarounds. Gate: `tests/test_const_float_expr_diag.bpp` (xfail). |
| E269 | stray identifier at file scope | bpp_parser.bsm | _diag_stray_ident | ❌ | 2026-07-07 storage cleanup: a bare `x;` (or even `x` — the semicolon was optional!) at file scope used to declare a global via the legacy-B `parse_global_simple` shorthand, so any typo'd keyword or leftover word silently became a variable and the program compiled. Zero uses existed in the repo; the shorthand is removed and a stray identifier is now heard. This flushed a real latent typo the old swallow had hidden for months — `stbgame.bsm:2` was `/f/ Initializes…` (a mistyped `//`), silently eaten. Help lines point at `auto`/`global`/`extrn` and the misspelled-keyword possibility. Gate: `tests/test_e269_stray_identifier.bpp` (xfail). |
| E270 | `static extrn` is meaningless | bpp_parser.bsm | _diag_static_extrn | ❌ | 2026-07-07 storage cleanup: `static` marks a module-private DEFINITION, but `extrn` is a REFERENCE to a definition living elsewhere — visibility belongs to the defining module, so the prefix asserts nothing. It parsed silently for months (the `static` was swallowed with no effect) and 301 declarations across the tree wore it; the cleanup swept them to plain `extrn` and made the spelling a diagnostic (recovers by parsing as a plain extrn). Gate: `tests/test_e270_static_extrn.bpp` (xfail). |
| E271 | _(retired same day, 2026-07-07 — superseded by distinct slots)_ static global aliasing across modules | — | — | 🗑 | Lived across two commits during the storage cleanup. Phase F2a briefly REJECTED a `static` global that would alias a same-named different-module global (same-named globals shared one data slot). Phase F2b made it WORK instead: `mangle_private_globals` gives each colliding `static` its own distinct slot via a mangled internal name (`<name>__mp<module>`) + rewrites in-module references, so two modules can each hold a private `static x` without aliasing. With real separation, the rejection was wrong — E271 and its check were removed and the gate became the positive `tests/test_module_private_globals.bpp`. Number burned, not reused. |
| E268 | unknown statement-level annotation | bpp_parser.bsm | parse_statement `@` handler | ❌ | 2026-07-07 annotation cleanup: `@par while (...)` — or any `@word` at statement level that isn't `@profile` — used to be SILENTLY swallowed as a vestigial "dispatch hint" nothing ever read (classify_loop compared the stored packed token against 1/2/3, which never matched, so `@seq`/`@par`/`@gpu` were dead on arrival; the branch and the parser plumbing are removed). A typo'd annotation must be heard, not eaten. Help lines state the whole real surface: `@profile(...)` at statement level, `@safe` on a function signature, loop dispatch automatic. Gate: `tests/test_e268_unknown_stmt_annotation.bpp` (xfail). |
| E265 | _(retired 2026-05-27)_ struct exceeds the per-struct field limit | — | — | 🗑 | Lived for one day. Introduced as a bounds-check + cap (256) over the fixed `buf_word(256)` per-struct field tables when the size-class free-list first exposed the overflow. Retired the same day when the per-struct field tables were migrated to a growable `arr_struct<FieldRec>` (one growable array per struct, no fixed cap) — the diagnostic can no longer fire because there is no buffer to overflow. The E265 number is burned, not reused. See the dh_arr/ph_arr/sd_* growable-buffer migration (the "fixed buffer + cap + E-net" anti-pattern eliminated). |
...
| W038 | integer literal in `: ptr` parameter position | bpp_validate.bsm | 1030 | ✅ | Callsite passes a non-zero integer literal to a parameter declared `: ptr`. The canonical bug class: param-order swap where a `const` enum value (e.g. `BLDG_GOLD_MINE = 9`) lands in a handle slot, and the callee then SIGSEGVs deep inside the deref. Mirror of E232 / W032 shape (silent type-confusion at call boundaries). Narrow gate: only INT LITERAL triggers — TY_WORD-typed vars holding pointers do NOT warn today (Rule 13 nudge belongs in a future audit). Literal `0` skipped (canonical NULL sentinel). Originating session: rts1 S9 `ecs_spawn_building` param-order bug 2026-05-20 (would have been caught at compile time had W038 existed). E050's `: ptr` → `: <Struct>` strictness was relaxed the same session — typed-struct locals are pointer-addressed under the hood so the assignment IS bit-pattern-compatible; the destination annotation is the user's contract. That relaxation unblocked annotating `: ptr` on the stbimage public draw / load surface (`image_draw_size` family, `image_load`, `image_anim_id`, etc.). |
| W033 | `@safe` loop shaped for auto-dispatch never qualified | bpp_dispatch.bsm | `warn_unfired_safe_loops` | ✅ | A `@safe` function contains a top-level loop matching the dispatch scanner's loose shape (ascending `i = i + K`, K > 0 literal, condition `i < bound`) that never made it into `dispatch_candidates` — the annotation promised "this can run on a worker" but the body was rejected (an unpromoted global, an impure call, an unsupported sub-shape) with zero compiler feedback otherwise. Originating bug: `examples/bench_autovec_compute.bpp`'s `parallel_once` — `[dispatch] 0 candidate loop(s)`, discoverable only via `--dispatch`, not via any default diagnostic. Deliberately does NOT fire when the loop WAS accepted into `dispatch_candidates` and a later stage (the `_DSP_MAP_MIN_N` / `_OUTLINE_MIN_N` floor guards) correctly kept it serial for a small trip count — that is the optimizer working as intended, not a bug. ALSO skips any function whose address is taken (`fn_address_taken`) — already registered as a callback / job target elsewhere (CoreAudio's `_aud_square_cb` / `_aud_stream_cb`, `job_submit(fn_ptr(particle_base_job), ...)`), where `@safe` exists for the W026 bounded-contract, not for auto-dispatch — and any loop whose body contains a nested `T_WHILE`/`T_SWITCH` (`mixer_fill`'s outer per-sample loop wrapping an inner per-voice loop), a documented MVP limitation (how_to_dev_b++.md §5.6: "no nested loops") rather than a surprise. All three exclusions were found as real false positives during this diagnostic's own verification sweep across every game/tool in the repo, not hypothetical. |
