...
| ↳ M2 caller-saved spilling (locals promoted into a caller-saved band in non-leaf fns, saved/restored around each call site) | ✅ x9..x12, +4 slots (2026-07-09) | ✅ r8/r9, +2 slots (2026-07-09 — leaf keeps them as freelist temps, the inverse of a64's leaf widening; inline syscalls get their own chip-local wrap since sys_mmap/sys_select pop args into r8/r9 outside cg_emit_call) | parity (band width differs by ABI headroom) — M3-gated on both chips (refs > 2× loop-weighted call count, policy shared in the spine); a64: manylive 1.44x→1.26x, biquad/lcg beat gcc -O2. Since 2026-07-10 the wrap is live-range MASKED on both chips (M5, shared spine stamp): a call skips band entries whose occupant is provably dead there; the x64 inline-syscall self-wrap stays full-band |
